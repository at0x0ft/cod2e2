...
reset_extension_volumes() {
  ...
  local readonly runtime_container_name=$(get_docker_container_name ${RUNTIME_CONTAINER_SERVICE_NAME})
  ...
  if docker_container_exists "${runtime_container_name}"; then
    docker container rm -f "${runtime_container_name}"
  fi
